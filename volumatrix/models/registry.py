    ...
    **kwargs,
    ...
    with _registry_lock:
        ...
        if name in _model_registry:
            log.warning(f"Overwriting existing model '{name}' in registry")
        ...
        log.info(f"Registered model '{name}' ({model_class.__name__})")
...
def unregister_model(name: str) -> bool:
    ...
    with _registry_lock:
        ...
        if _default_model == name:
            _default_model = (
                next(iter(_model_registry.keys())) if _model_registry else None
            )
...
def set_default_model(name: str) -> bool:
    ...
    with _registry_lock:
        ...
        _default_model = name
        log.info(f"Set default model to '{name}'")
        return True
...
def clear_registry() -> None:
    ...
    with _registry_lock:
        ...
        log.info("Cleared model registry")
...
def auto_discover_models() -> List[str]:
    ...
    log.info(f"Auto-discovered {len(discovered)} models: {discovered}")
    return discovered
...
def load_model(name: Optional[str] = None) -> bool:
    ...
    try:
        ...
        log.error(f"Failed to load model '{model.name}': {e}")
        return False
...
def unload_model(name: Optional[str] = None) -> bool:
    ...
    try:
        model.unload()
        log.info(f"Unloaded model '{model.name}'")
        return True
    except Exception as e:
        log.error(f"Failed to unload model '{model.name}': {e}")
        return False
...
def _initialize_registry():
    ...
    try:
        ...
        log.warning(f"Failed to auto-discover models: {e}")
