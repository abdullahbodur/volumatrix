...
def preview(obj: Union[VolumatrixObject, Scene], 
    ...
    if backend == "pyvista":
        _preview_pyvista(obj, window_title=window_title, window_size=window_size, auto_show=auto_show, **kwargs)
    elif backend == "trimesh":
        _preview_trimesh(obj, window_title=window_title, auto_show=auto_show, **kwargs)
    elif backend == "plotly":
        _preview_plotly(obj, window_title=window_title, window_size=window_size, **kwargs)
    elif backend == "matplotlib":
        _preview_matplotlib(obj, window_title=window_title, window_size=window_size, **kwargs)
    else:
...
def _select_best_backend() -> str:
    ...
    raise RuntimeError("No visualization backend available. Install pyvista, trimesh, plotly, or matplotlib.")
...
def _preview_pyvista(obj: Union[VolumatrixObject, Scene], 
    ...
    try:
        ...
        logger.error("PyVista is required for pyvista backend. Install with: pip install pyvista")
        raise
    ...
    try:
        ...
        if meshes:
            if auto_show:
                ...
                logger.info("Trimesh visualization prepared but not shown (auto_show=False)")
        else:
            logger.warning("No mesh representations found for preview")
            
    except ImportError:
        logger.error("trimesh is required for trimesh backend. Install with: pip install trimesh")
        raise
...
def _preview_plotly(obj: Union[VolumatrixObject, Scene], 
    ...
    try:
        ...
        logger.error("plotly is required for plotly backend. Install with: pip install plotly")
        raise
...
def _preview_matplotlib(obj: Union[VolumatrixObject, Scene], 
    ...
    try:
        ...
        logger.error("matplotlib is required for matplotlib backend. Install with: pip install matplotlib")
        raise
...
def _add_object_pyvista(plotter, obj: VolumatrixObject, name: Optional[str] = None) -> None:
    ...
    if obj.mesh is not None:
        ...
        if pc.colors is not None:
            ...
            plotter.add_mesh(pv_points, name=display_name, scalars="colors", point_size=5, render_points_as_spheres=True)
        else:
            plotter.add_mesh(pv_points, name=display_name, color="red", point_size=5, render_points_as_spheres=True)
        ...
        if len(coords) > 0:
            ...
            # Create small cubes
            cubes = pv_points.glyph(geom=pv.Cube(), scale=False, factor=voxel.spacing * 0.8)
...
def _preview_pythreejs(obj: Union[VolumatrixObject, Scene], **kwargs) -> None:
    ...
    try:
        ...
        logger.error("pythreejs is required for pythreejs backend. Install with: pip install pythreejs")
        raise
...
def _preview_k3d(obj: Union[VolumatrixObject, Scene], **kwargs) -> None:
    ...
    try:
        ...
        logger.error("k3d is required for k3d backend. Install with: pip install k3d")
        raise 