...
def preview(
    obj: Union[VolumatrixObject, Scene],
    backend: str = "auto",
    ...
    **kwargs,
    ...
    if backend == "pyvista":
        ...
        _preview_plotly(
            obj, window_title=window_title, window_size=window_size, **kwargs
        )
...
def preview_jupyter(
    obj: Union[VolumatrixObject, Scene], backend: str = "plotly", **kwargs
) -> None:
    """
...
def _preview_pyvista(
    ...
    **kwargs,
    ...
    try:
        ...
        # Create plotter
        plotter = pv.Plotter(
            title=window_title, window_size=window_size, off_screen=not auto_show
        )
    ...
    **kwargs,
    ...
    try:
        ...
        if meshes:
            ...
            log.warning("No mesh representations found for preview")
...
def _add_object_pyvista(
    plotter, obj: VolumatrixObject, name: Optional[str] = None
) -> None:
    ...
    if obj.mesh is not None:
        ...
        if len(coords) > 0:
            ...
            # Create small cubes
            cubes = pv_points.glyph(
                geom=pv.Cube(), scale=False, factor=voxel.spacing * 0.8
            )
            plotter.add_mesh(cubes, name=display_name, color="green", opacity=0.7)

    else:
        log.warning(f"No supported representation found for object: {obj.name}")


def _plot_object_matplotlib(
    ax, obj: VolumatrixObject, label: Optional[str] = None
) -> None:
...
def _preview_pythreejs(obj: Union[VolumatrixObject, Scene], **kwargs) -> None:
    ...
    try:
        ...
        log.warning("pythreejs preview is not fully implemented yet")
...
def _preview_k3d(obj: Union[VolumatrixObject, Scene], **kwargs) -> None:
    ...
    try:
        ...
        log.warning("k3d preview is not fully implemented yet")

        # Fallback to plotly
        _preview_plotly(obj, **kwargs)

    except ImportError:
        log.error("k3d is required for k3d backend. Install with: pip install k3d")
        raise
