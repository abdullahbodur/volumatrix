...
def voxelize(obj: VolumatrixObject, resolution: int = 64,
  ...
  if isinstance(primary_repr, Mesh):
    ...
    raise ValueError(
      f"Cannot voxelize representation type: {type(primary_repr)}")
  ...
  if method == "marching_cubes":
    try:
      ...
      spacing_array = np.array(
        [voxel_repr.spacing] * 3) if isinstance(voxel_repr.spacing, float) else voxel_repr.spacing
...
def _simple_devoxelize(voxel: Voxel) -> tuple[np.ndarray, np.ndarray]:
  ...
  spacing_array = np.array(
    [voxel.spacing] * 3) if isinstance(voxel.spacing, float) else voxel.spacing
...
def _sample_mesh_surface(mesh: Mesh, num_points: int) -> tuple[np.ndarray, Optional[np.ndarray], Optional[np.ndarray]]:
  ...
  sampled_faces = np.random.choice(
    len(mesh.faces), num_points, p=face_probabilities)
