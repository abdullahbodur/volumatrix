...
def generate(
    ...
    **kwargs,
    ...
    try:
        ...
        log.info(
            f"Generating 3D object with prompt: '{prompt}' using model: {model_instance.name}"
        )
        ...
        log.info(f"Successfully generated object: {result}")
        ...
        if auto_preview:
            try:
                ...
                log.info(f"Auto-previewing generated object: {result.name}")
                preview(
                    result, backend=preview_backend, window_title=f"Generated: {prompt}"
                )
            except Exception as e:
                log.warning(f"Failed to auto-preview object: {e}")

        return result

    except Exception as e:
        log.error(f"Failed to generate object with prompt '{prompt}': {e}")
        raise
...
def generate_batch(
    ...
    **kwargs,
    ...
    log.info(f"Starting batch generation of {len(prompts)} objects")
    ...
    if auto_preview or max_workers == 1:
        ...
        for i, (prompt, task_kwargs) in iterator:
            try:
                ...
                log.error(f"Failed to generate object {i} with prompt '{prompt}': {e}")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            if show_progress:
                completed_futures = tqdm(
                    completed_futures, total=len(tasks), desc="Generating objects"
                )
            ...
            for future in completed_futures:
                ...
                try:
                    ...
                    log.error(
                        f"Failed to generate object {i} with prompt '{prompt}': {e}"
                    )
                    failed_tasks.append((i, prompt, str(e)))
                    results[i] = None
    ...
    log.info(
        f"Batch generation completed: {successful_count}/{len(prompts)} successful"
    )

    if failed_tasks:
        log.warning(f"Failed to generate {len(failed_tasks)} objects:")
        for i, prompt, error in failed_tasks:
            log.warning(f"  {i}: '{prompt}' - {error}")
