...
def generate(
  ...
  try:
    ...
    if not isinstance(result, VolumatrixObject):
      raise TypeError(
        f"Model returned {type(result)}, expected VolumatrixObject")
    ...
    if auto_preview:
      try:
        ...
        preview(result, backend=preview_backend,
                window_title=f"Generated: {prompt}")
...
def generate_batch(
  ...
  if auto_preview or max_workers == 1:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
      ...
      if show_progress:
        completed_futures = tqdm(completed_futures, total=len(
          tasks), desc="Generating objects")
