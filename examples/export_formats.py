...
def basic_export_formats():
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    print(
      f"Exported to OBJ: {obj_file.name} ({obj_file.stat().st_size} bytes)")

    # Export to STL format
    stl_file = temp_path / "chair.stl"
    vm.export(chair, str(stl_file))
    print(
      f"Exported to STL: {stl_file.name} ({stl_file.stat().st_size} bytes)")
    ...
    try:
      ...
      print(f"PLY export not available: {e}")
...
def export_error_handling():
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    try:
      ...
      print(f"Unknown format error (expected): {e}")
